...
contract LMSRMarket {
    ...
    /// @param _b USDC units (6d). Require subsidy >= b*ln(2) for solvency.
    /// @param _initialLiquidity pulled from msg.sender (approve first).
    constructor(address _usdc, string memory _q, uint256 _end, uint256 _b, uint256 _initialLiquidity) {
        require(_usdc != address(0), "usdc");
        require(_b > 0, "b");
        require(_end > block.timestamp, "end");
        // require(initialLiquidity >= b * ln(2))
        require(_initialLiquidity >= (uint256(_b) * uint256(LN2_WAD)) / 1e18, "subsidy<b*ln2");
        ...
        if (_initialLiquidity > 0) {
            IERC20(_usdc).safeTransferFrom(msg.sender, address(this), _initialLiquidity);
        }
    ...
    function priceNo() public view returns (uint256) { return 1e18 - priceYes(); }

    // ---- quotes (inputs: dQ WAD shares; outputs: USDC 6d) ----
    function quoteBuyYes(uint256 dQ) external view returns (uint256) { require(dQ>0,"dQ"); return _cost(qYes+dQ, qNo) - _cost(qYes, qNo); }
    function quoteBuyNo (uint256 dQ) external view returns (uint256) { require(dQ>0,"dQ"); return _cost(qYes, qNo+dQ) - _cost(qYes, qNo); }
    function quoteSellYes(uint256 dQ) external view returns (uint256) { require(dQ>0 && dQ<=qYes,"dQ"); return _cost(qYes, qNo) - _cost(qYes-dQ, qNo); }
    function quoteSellNo (uint256 dQ) external view returns (uint256) { require(dQ>0 && dQ<=qNo ,"dQ"); return _cost(qYes, qNo) - _cost(qYes, qNo-dQ); }
    ...
    function buyYes(uint256 dQ) external active {
        ...
        qYes += dQ; yesShares[msg.sender] += dQ;
    ...
    function sellYes(uint256 dQ) external active {
        ...
        yesShares[msg.sender] -= dQ; qYes -= dQ;
    ...
    function claim() external {
        ...
        if (outcome == Outcome.Yes) {
            uint256 u = yesShares[msg.sender]; require(u>0, "no win");
            yesShares[msg.sender] = 0;
            uint256 payout = (u * ONE_USDC) / 1e18; // 1 USDC per share
            usdc.safeTransfer(msg.sender, payout);
            emit Claimed(msg.sender, payout);
        } else {
            uint256 u = noShares[msg.sender]; require(u>0, "no win");
